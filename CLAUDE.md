...
## Current state: v0.7.0 shipped; v0.7 Track 2 (LoRA adapter multiplexing) in `sheaf.lora`, wired into Ray Serve + Modal paths
...
- **LoRA adapter multiplexing** — opt-in per deployment via `ModelSpec.lora = LoRAConfig(adapters={...}, default="...")`. Diffusion backends (FLUX, SDXL) implement the `supports_lora()` / `load_adapters()` / `set_active_adapters()` hooks on `ModelBackend`. Adapters are loaded once at deploy time; per-request selection is via `DiffusionRequest.adapters` / `MultimodalGenerationRequest.adapters` (with optional `adapter_weights` override). `_SheafDeployment` automatically buckets requests by their resolved `(names, weights)` tuple inside each Ray Serve batch window so `set_active_adapters` is called exactly once per homogeneous sub-batch.
...
### LoRA adapter multiplexing (v0.7 Track 2)

- **`LoRAAdapter.source` parsing — `hf:` prefix means HF Hub** — local paths and HF Hub IDs are distinguished by a single prefix: `"hf:org/repo"` resolves to a HuggingFace repo, `"hf:org/repo:weight_file.safetensors"` pins a specific weight file, anything else is treated as a local path. `sheaf.lora.parse_source(source)` returns `(path_or_repo, weight_name)` and is used by both FLUX and SDXL backends. The choice to fold the weight-name pinning into one string (rather than adding a `weight_name` field on `LoRAAdapter`) keeps the spec minimal — the colon-separated form is the same shape diffusers' `load_lora_weights` consumes internally, so users who already know diffusers don't have to learn a new schema.
- **Adapter weight resolution rules** — request weights win when set; otherwise fall back to per-adapter `LoRAAdapter.weight` from the spec; otherwise 1.0. Resolution lives in `sheaf.lora.resolve_active_adapters(request, lora_config)` so the same logic is shared between the Ray Serve dispatch (`_batch_predict`) and the Modal handler. Two requests that resolve to the same `(names, weights)` tuple — e.g. one with empty `adapters` + spec default `"sketch"`, one with explicit `adapters=["sketch"]` and the same default weight — share a bucket so adapter activation happens once.
- **Bucket-by-resolved-adapter is implicit when `spec.lora` is set** — there is no user-facing `bucket_by_adapter` flag; whenever `ModelSpec.lora is not None`, requests are automatically grouped by their resolved adapter selection inside `_batch_predict` and `set_active_adapters` is called once per group. This is non-negotiable because `pipeline.set_adapters` is process-global state on the diffusers pipeline — concurrent requests inside the same Ray Serve batch window with different adapters would race. Combining `ModelSpec.lora` with `BatchPolicy.bucket_by` is rejected at spec construction time (the LoRA bucketing replaces the `bucket_by` field grouping for v1).
- **Adapters loaded once at deploy time, no hot-add** — `ModelBackend.load_adapters(adapters)` is invoked exactly once by `_SheafDeployment.__init__` (and the Modal `_build_asgi_app`) after `backend.load()`. Adding a new adapter requires `ModelServer.update(spec)` (which performs Ray Serve's standard rolling deployment update). Hot-add at runtime is intentionally out of scope — it would either require draining all in-flight requests (defeats the purpose) or risk index-out-of-sync state between the worker registry and the pipeline's internal adapter map.
- **`supports_lora()` opt-in is required** — `ModelBackend.supports_lora()` defaults to `False`; backends must override and return `True` to receive `load_adapters` / `set_active_adapters` calls. `_SheafDeployment.__init__` raises `ValueError` if `spec.lora is not None` and `backend.supports_lora() is False`. This is a deploy-time error, not a request-time one — surfacing it where the misconfigured spec is constructed, not after a request comes in.
- **Stream endpoint applies adapters per-request** — `POST /{name}/stream` bypasses batching but still calls `set_active_adapters` before `backend.stream_predict()`. Since streaming is per-request by design and `pipeline.set_adapters` is global state, any concurrent streams against the same deployment with different adapters would race; the streaming contract is "one stream at a time per replica" already (it does not benefit from `@serve.batch`), so the same operational caveat as Modal's `allow_concurrent_inputs > 1` applies.
- **Modal's `allow_concurrent_inputs` caveat** — `ModalServer._build_asgi_app` calls `set_active_adapters` inline before `backend.async_predict`. Modal's default concurrency is 1 (one request at a time per container slot), so this is safe. Cranking `allow_concurrent_inputs > 1` without sharding adapter selection across replicas creates the same race as Ray Serve does without bucket-by-adapter — caller's responsibility, documented in the helper docstring (`modal_server._apply_lora`).
